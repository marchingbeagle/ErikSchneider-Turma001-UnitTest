class AnaliseDeDados {
  ...
  calcularVariancia() {
    ...
    return (
      this.dados.reduce((sum, num) => sum + Math.pow(num - media, 2), 0) /
      this.dados.length
    );
  }
  ...
  calcularPercentil(percentil) {
    if (this.dados.length === 0 || percentil < 0 || percentil > 100)
      return null;
    const ordenado = this.ordenarDados();
  ...
  removerOutliers(fator = 1.5) {
    ...
    this.dados = this.dados.filter(
      (num) => num >= limiteInferior && num <= limiteSuperior
    );
  }

  calcularCorrelacao(outroConjunto) {
    if (
      !Array.isArray(outroConjunto) ||
      this.dados.length !== outroConjunto.length
    )
      return null;

    const mediaX = this.calcularMedia();
    const mediaY =
      outroConjunto.reduce((sum, num) => sum + num, 0) / outroConjunto.length;

    const numerador = this.dados.reduce(
      (sum, x, i) => sum + (x - mediaX) * (outroConjunto[i] - mediaY),
      0
    );
    const denominadorX = Math.sqrt(
      this.dados.reduce((sum, x) => sum + Math.pow(x - mediaX, 2), 0)
    );
    const denominadorY = Math.sqrt(
      outroConjunto.reduce((sum, y) => sum + Math.pow(y - mediaY, 2), 0)
    );

    return denominadorX * denominadorY !== 0
